...
import NodeSelectionStore, {
  ISelectableNode,
  ISelectableNodeMeta
} from "../../stores/nodes/NodeSelection.store";
...
export const BaseNode: React.FC<BaseNodeProps> = ({
  ...
  const controls = useBaseNodeControls(
    label,
  ...
  return (
    <>
      <mesh
        ...
        receiveShadow
        position={[
          axisValueSnapper(controls.x),
          defaultY,
          axisValueSnapper(controls.z)
        ]}
        onPointerDown={e => {
          if (!selectedNode) {
      ...
      >
        ...
        {showLabel && (
          <Html scaleFactor={0.018} className="">
            <div
              className={`transform -translate-y-32 -translate-x-20 ${labelClass}`}
            >
              <ScaleUp on={hovered}>
                <NodeTag {...meta} />
              </ScaleUp>
